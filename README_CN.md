...
> 文档同步规则：当代码、配置、调度行为或能力边界发生变化时，必须在同一批改动中同步更新本 README 及受影响的本地化文档。
>
> Fork 分支协作规则：如果当前仓库需要长期跟踪上游，请把 `main` 保留为同步 `upstream/main` 的基线分支；本地定制和日常开发放在独立的集成分支（例如 `integrate/...`）上，先更新 `main`，再把上游变化合入或 rebase 到集成分支。
...
- **管理后台** - Web 界面进行监控和管理
- **OpenAI Free 分池与重置感知调度** - 提供管理侧 OpenAI free 分池工具、仪表盘未来重置分布，以及基于 `extra.codex_7d_reset_at` 的 `账号池` 运行时优先调度能力
- **外部系统集成** - 支持通过 iframe 嵌入外部系统（如工单等），扩展管理后台功能

### 管理侧 OpenAI Free 分池

管理后台、账号表格和独立的 `OpenAI Free 分池` 页面包含一套面向源码部署定制的 OpenAI free 账号管理流程：

- 适用范围：仅 `platform=openai` 且 `credentials.plan_type=free` 的账号
- 配置结构：一个 `default` 收容分组、一个排除用 `plus` 分组，以及固定五个 `{group, proxy}` 分池绑定
- 预览与应用：独立页面默认自动加载预览，支持手动应用；默认仅移动仍在 `default` 分组中的新 free 账号；目标分池会尽量让相同 `codex_7d_reset_at` 日期的账号进入同一池，已稳定的账号默认保持原位，除非显式启用强制重平衡
- 手动锁定：运维可以把受管 OpenAI free 账号手动锁定到五个分池之一；所绑定代理会自动随分池切换；解锁只移除人工覆盖，不会强制立即迁移
- 未来重置分布：仪表盘卡片按 `extra.codex_7d_reset_at` 聚合未来重置时间，绘制折线图，并支持按日期展开账号明细；缺少该字段的账号会归入 `Unknown`
- 状态反馈：当管理员测试账号返回 OAuth 风格的 `401` / `token_invalidated` 错误时，前端会立即回刷该账号；如果后端已落库为 `error`，界面会立刻切换徽标状态。手动 `Refresh Token` 也会给出明确成功/失败提示，并在失败后刷新该行状态

### OpenAI Free 重置感知调度

在运行时请求路径中，Sub2API 现在可以在分组名严格等于 `账号池` 的组内，优先选择“更快重置”的 OpenAI free 账号。

- 激活条件：仅对 `platform=openai`、`credentials.plan_type=free` 且 `extra.codex_7d_reset_at` 为合法 RFC3339 时间的账号生效
- 预过滤：所有不可调度账号都会先被过滤掉，包括仍处于 `rate_limit_reset_at` 冷却窗口内的账号
- 排序规则：在完成过滤后，调度仍保留原有分层逻辑，并把 reset 优先级插入为额外排序条件。负载感知路径为 `priority -> 负载率 -> 更早的 OpenAI free reset -> last used / 同层随机`；fallback `random` 模式保留 `priority -> 更早的 OpenAI free reset`，只在完全同层候选里随机
- 边界约束：该规则不会改变其他分组的调度，不会覆盖 `priority`，也不会作用到非 free 或非 OpenAI 账号

当前边界：

- OpenAI Free 分池仍是管理侧工作流，不包含后台定时自动执行
- 后台 token 刷新不是完整的账号健康巡检；未被使用、未手测、未手动刷新过的账号，界面上仍可能继续显示为 `active`
- 上述运行时重置感知调度是一个有意收窄的规则：它只改变 `账号池` 组内、已经通过可调度校验的候选账号排序，并依赖正确的 `plan_type` 与 `codex_7d_reset_at` 元数据
- 源码级测试可以验证规划逻辑、接口契约和前端渲染，但不能直接证明生产环境映射、真实账号元数据完整性，以及对现网运行实例的零影响
