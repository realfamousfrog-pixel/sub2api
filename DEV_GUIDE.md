...
### 坑 11：只想本地看前端管理页，但没有启动后端

**场景**：
- 只启动了 `frontend` 的 Vite 开发服务；
- 没有启动后端、PostgreSQL、Redis；
- 需要直接进入管理页查看前端改动。

**当前做法**：
1. 进入 `frontend` 目录并启动开发服务：
   ```bash
   pnpm run dev
   ```
2. 打开 `http://localhost:5173/login`
3. 在登录页点击仅开发环境可见的 `进入开发预览管理后台` 按钮
4. 页面会写入本地假管理员登录态，并使用前端内置 mock 数据进入 `/admin/accounts`

**边界说明**：
- 该入口只在 `import.meta.env.DEV` 下显示，不影响生产构建
- 该模式只用于查看前端页面与交互，不代表真实后端行为
- 当前 mock 覆盖了账号页、仪表盘以及顶部常用管理接口，足够查看 OpenAI Free 分池和重置看板改动
