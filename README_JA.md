...
> ドキュメント同期ルール: コード、設定、スケジューリング動作、または機能境界が変更された場合は、この README と影響を受けるローカライズ文書を同じ変更で更新してください。
>
> Fork ブランチ運用ルール: 上流を継続的に追従する fork では、`main` を `upstream/main` 同期専用の基準ブランチとして維持してください。ローカルのカスタマイズや日常開発は別の統合ブランチ（例: `integrate/...`）で行い、`main` を更新した後にその変更を統合ブランチへ merge または rebase します。
